...
pCFGNode::pCFGNode(const vector<DataflowNode>& pSetDFNodes, const vector<std::pair<string, int> > visited, const int prev_send, const int prev_recv, const vector<struct match> matchings, const vector<vector<DataflowNode> > sendSets)
{
        this->pSetDFNodes = pSetDFNodes;
        this->visited = visited;
        this->prev_send = prev_send;
        this->prev_recv = prev_recv;
        this->matchings = matchings;
        this->sendSets = sendSets;
}
...
void pCFGNode::operator = (const pCFGNode& that)
{
        ...
        prev_send = that.prev_send;
        prev_recv = that.prev_recv;
        matchings = that.matchings;
        sendSets = that.sendSets;
        // modifications for dot printing
        // ancspCFGNodes = that.ancspCFGNodes;    // keep passing the known ancestors
        // dot_name = that.dot_name;
        // dot_attr = that.dot_attr;
}
...
void pCFGNode::advanceOut(unsigned int pSet)
{
        ...
        // string s = pSetDFNodes[pSet].toString();
        // int pos = s.find("pcfg_match");
        // if(pos >= 0){
        //     DataflowNode df = edges[0].target();
        //     SgNode* sgn = df.getNode();
        //     pcfgMatchAnnotation *Annotation = dynamic_cast<pcfgMatchAnnotation*> (sgn->getAttribute("pCFGAnnotation"));
        //     pair<string, int> Source = Annotation->getSource();
        //     pair<string, int> Target = Annotation->getTarget();
        //     if(Source.second == -1){
        //         for(int i=0;i<mpi_calls_seen.size();i++){
        //             pcfgMatchAnnotation* pcfgannotation = new pcfgMatchAnnotation(pcfg_match);
        //             pcfgannotation->setSource(Source.first, 0);
        //         }
        //     }
        //     mpi_calls_seen.push_back(Source);
        
        // }
        // cout<<"\nyes = " << pSetDFNodes[pSet].toString() << "\n\n";
        ROSE_ASSERT(0<=pSet && pSet < pSetDFNodes.size());
        if(pCFGNode::nodestack[pSet].empty()){
            return;
        }
        ...
        visited.push_back(make_pair(pSetDFNodes[pSet].id(), pSet));

        for(int i=edges.size()-1;i>=0;i--){
            DataflowNode dfnode = edges[i].target();
            pair<string, int> p = pair<string,int>(dfnode.id(),pSet);
            // cout << "edges.size = "<<edges.size() << endl;
            if (std::find(visited.begin(), visited.end(),p) == visited.end()){
                pCFGNode::nodestack[pSet].push(dfnode);
            }
            else{
                // vector<DataflowEdge> edges2 = pSetDFNodes[pSet].inEdges();
                // pSetDFNodes[pSet] = (*(edges2.begin())).source();
                // edges2 = pSetDFNodes[pSet].outEdges();
                // for(int i=0;i<edges.size();i++){
                // DataflowNode dfnode = edges[i].target();
                // if (std::find(visited.begin(), visited.end(),dfnode.id()) == visited.end()){
                //     pSetDFNodes[pSet] = dfnode;//(*(edges.begin())).target();
                //     visited.push_back(dfnode.id());
                //     break;
                // }
                cout<<"good "<<edges.size()<<endl;
            }
        }
        DataflowNode node = pCFGNode::nodestack[pSet].top();
        pCFGNode::nodestack[pSet].pop();
        
        pSetDFNodes[pSet] = node;//(*(edges.begin())).target();
}
...
{
        // cout << "erur: pSet = " << pSet << endl;
        // cout << "erur: pSetDFNodes.size() = " << pSetDFNodes.size() << endl;
        ROSE_ASSERT(0<=pSet && pSet<pSetDFNodes.size());
...
pCFGNode& pCFGNode::getCurAncsNode(unsigned int pSet)
{
    ROSE_ASSERT(pSet >= 0 && pSet < ancspCFGNodes.size());
     return ancspCFGNodes[pSet];
}

// void pCFGNode::removeAncsNode(unsigned int pSet)
// {
//     ROSE_ASSERT(pSet >= 0 && pSet < ancspCFGNodes.size());
//     ancspCFGNodes.erase(ancspCFGNodes.begin() + pSet);
// }

// int pCFGNode::createAncsNode(const pCFGNode& _pcfgnode)
// {
//     // ancspCFGNodes.push_back(_pcfgnode);
//     // return ancspCFGNodes.size()-1;
// }

void pCFGNode::updateAncsNode(unsigned int pSet, const pCFGNode& _pcfgancs)
{
    ROSE_ASSERT(pSet >= 0);
    // add new entry
    if(pSet >= ancspCFGNodes.size()) {
        ROSE_ASSERT(pSet == ancspCFGNodes.size()); // only add a new entry for the next pset
        ancspCFGNodes.push_back(_pcfgancs);
    }
    else {
        // update
        ancspCFGNodes[pSet] = _pcfgancs;
    }
}

void pCFGNode::copyAncsNode(const pCFGNode& that_pcfg)
{
    // copy only the ancestors
    this->ancspCFGNodes = that_pcfg.ancspCFGNodes;
}
...
void pCFGNode::set_dot_attr(string color)
{
    ostringstream outs;
    int curpset = 0;
    outs << "[label=\" ";
    for(vector<DataflowNode>::iterator it = pSetDFNodes.begin(); it!= pSetDFNodes.end(); curpset++) {
        outs << "< " << curpset << ", " << (*it).getNode()->class_name() << ", " ;
        string unparse = (*it).getNode()->unparseToString();
        string unparse_format_string = unparse.size() < 20 ? unparse : unparse.substr(0, 20);
        outs << " \'\' " << unparse_format_string << " \'\' >";
        it++;
        if(it != pSetDFNodes.end())
            outs << "\\n\\n";
    }
...
void pCFGNode::set_dot_attr(unsigned int curPSet)
{
    ostringstream outs;
    ROSE_ASSERT(curPSet >= 0 && curPSet < pSetDFNodes.size());
    
    string class_name = pSetDFNodes[curPSet].getNode()->class_name();
    string unparse = pSetDFNodes[curPSet].getNode()->unparseToString();
    string unparse_format_string = unparse.size() < 20 ? unparse : unparse.substr(0, 20);
    outs << "[label=\"" << "< " << curPSet << ", " << class_name << ", " << unparse_format_string << " >";
    outs << "\"]";
    dot_attr = outs.str();
}
...
string pCFGNode::id()
{
    ROSE_ASSERT(dot_name.size() > 0);
    return dot_name;
}

string pCFGNode::get_dot_attr()
{
    // ROSE_ASSERT(dot_attr.size() > 0);
    return dot_attr;
}

void pCFGNode::set_id(int name)
{
    ostringstream outs;
    outs << name;
    dot_name = outs.str();
...
string pCFGNode::toDot()
{
    ostringstream outs;
    outs << id()  << " " << get_dot_attr() << ";" << endl;
    return outs.str();
}

// string pCFGNode::toDot(unsigned int curPSet)
// {
//     ostringstream outs;
//     outs << id() << " " << get_dot_attr() << ";";
//     return outs.str();
// }
