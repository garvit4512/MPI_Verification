...
struct match{
    ...
    match(string s, int sid, string r, int rid){
        this->sender = s;
        this->send_id = sid;
        this->receiver = r;
        this->recv_id = rid;
...
class pCFGNode
{
        ...
        public:
        static vector<std::stack<DataflowNode> > nodestack;
        static std::stack<vector<std::stack<DataflowNode> > > bigstack;
        vector<std::pair<string, int> > visited;
        // vector<std::pair<string, int> > mpi_calls_seen;
        int prev_send;
        int prev_recv;
        vector<struct match> matchings;
        vector<DataflowNode> pSetDFNodes;
        vector<vector<DataflowNode> > sendSets;
        vector<pCFGNode> ancspCFGNodes;
        pCFGNode();
        //~pCFGNode() { cout << "destructor called"; }
        pCFGNode(const vector<DataflowNode>& pSetDFNodes, const vector<std::pair<string, int> > visited, const int prev_send, const int prev_recv, const vector<struct match> matchings, const vector<vector<DataflowNode> > sendSets);
        ...
        pCFGNode& getCurAncsNode(unsigned int pSet);

        // update ancesetor of pSet with _pcfgancs
        void updateAncsNode(unsigned int pSet, const pCFGNode& _pcfgancs);

        void copyAncsNode(const pCFGNode& that_pcfg);
        
        string str(string indent="") const;

        // dot representation of this object
        string toDot();
        ...
        // add return dot attributes for this node
        void set_dot_attr(string);
        void set_dot_attr(unsigned int);
        string get_dot_attr();
...
class pCFG_FWDataflow  : virtual public IntraProceduralDataflow
{
        ...
        virtual bool runAnalysis_pCFG(const Function& func, NodeState* fState, pCFG_Checkpoint* chkpt);
